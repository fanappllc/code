...
class NotificationsVC: UIViewController {
    ...
    func getNotificationList(pageCount:Int) {
        ...
        APIComponents.Customer.getNotifictionList(page: "\(pageCount)") {[weak self] (success, data, error) in
    ...
    func deleteAllNotifications(handler: @escaping() -> Void) {
        FanHUD.show()
        APIComponents.Customer.deleteAllNotifiction { [weak self] (success, error) in
            FanHUD.hide()
            guard let strongSelf = self else { return }
            guard success, error == nil else {
                if let message = error?.localizedDescription {
    ...
    func deleteNotification(notificationID : String, handler: @escaping() -> Void) {
        FanHUD.show()
        APIComponents.Customer.deleteNotifiction(notificationID: notificationID) { [weak self] (success, error) in
            FanHUD.hide()
            guard let strongSelf  = self else { return }
            guard success, error == nil else {
                if let message = error?.localizedDescription {
