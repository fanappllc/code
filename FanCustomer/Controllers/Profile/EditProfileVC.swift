...
//  FanCustomer
//
//  Created by Codiant on 11/14/17.
...
class EditProfileVC: UIViewController {
    ...
    @IBAction func btnSubmit_Action(_ sender: UIButton) {
        if sender.titleLabel?.text == "EDIT" {
            ...
            let parameters = ["first_name": txfFirstName.text!,"last_name": txfLastName.text!, "email": txfEmail.text!, "is_terms_condition_accepted" : "1", "address": self.txfAddress.text!, "zip_code": txfZipCode.text!]
            var images = [String: UIImage]()
            if let profileImage = imgViewProfile.image {
                images["profile_image"] = profileImage
            }
            FanHUD.show()
            APIComponents.Account.updateProfile(parameters: parameters, images: images) { [weak self] (success, data, error) in
                FanHUD.hide()
                ...
                if let object = data!.deserialize(), let data = object["data"] as? HTTPParameters {
                    ...
                    if !LoggedInUser.shared.isRegistered {
                        if let addCardVC = self?.storyboard?.viewController(withClass: AddCardVC.self) {
                            strongSelf.push(addCardVC)
                        }
                    } else {
                        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "REFRESH_USERINFO"), object: nil)
                        strongSelf.showAlertWith(message: "Profile successfully updated")
                        strongSelf.setProfileActive(false)
                    }
               }
            }
        }
