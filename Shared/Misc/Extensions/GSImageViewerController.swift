...
open class GSImageViewerController: UIViewController {
    ...
    override open func viewWillLayoutSubviews() {
        ...
        //scrollView.frame = (frame: CGRect(x: 0, y: 40, width: Screen.width, height: Screen.height - 90)) as! CGRect
...
//        let saveButton: UIButton = UIButton.init(frame: CGRect(x: 0, y: Screen.height - 50, width: Screen.width, height: 40))
//        saveButton.setTitle("SAVE", for: .normal)
//        saveButton.backgroundColor = Color.celrianBlue
//        saveButton.addTarget(self, action: #selector(save_Action(_:)), for: .touchUpInside)
//        self.view.addSubview(saveButton)
    ...
    fileprivate func setupScrollView() {
        ...
        let crossButton: UIButton = UIButton.init(frame: CGRect(x:Screen.width - 40, y: 20, width: 40, height: 40))
       
        crossButton.setImage(#imageLiteral(resourceName: "close"), for: .normal)
        crossButton.addTarget(self, action: #selector(dismiss_Action(_:)), for: .touchUpInside)
        view.addSubview(scrollView)
        if isSaveButtonEnabled!{
            let saveButton: UIButton = UIButton.init(frame: CGRect(x: 0, y: Screen.height - 45, width: Screen.width, height: 45))
            saveButton.setTitle("SAVE", for: .normal)
            saveButton.backgroundColor = Color.celrianBlue
            saveButton.addTarget(self, action: #selector(save_Action(_:)), for: .touchUpInside)
  ...
  //Akash
    @objc func save_Action (_ sender : UIButton) {
        
        self.dismiss(animated: true, completion: {
            self.callback()
        })
    }
    @objc func dismiss_Action (_ sender : UIButton) {
