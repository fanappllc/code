...
@IBDesignable open class AKSideMenu : UIViewController, UIGestureRecognizerDelegate {
    ...
    @IBInspectable open let contentViewStoryboardID: String? = nil
    @IBInspectable open let leftMenuViewStoryboardID: String? = nil
    @IBInspectable open let rightMenuViewStoryboardID: String? = nil
    @IBInspectable open var panFromEdgeZoneWidth : CGFloat = 20.0
    @IBInspectable open var panGestureLeftEnabled : Bool = true
    @IBInspectable open var panGestureRightEnabled : Bool = true
    @IBInspectable open var interactivePopGestureRecognizerEnabled: Bool = true
    @IBInspectable open var scaleContentView: Bool = true
    @IBInspectable open var fadeMenuView: Bool = true
    @IBInspectable open var backgroundTransformScale: CGFloat = 1.7
    @IBInspectable open var scaleBackgroundImageView: Bool = true
    @IBInspectable open var scaleMenuView: Bool = true
    @IBInspectable open var contentViewShadowEnabled: Bool = false
    @IBInspectable open var contentViewShadowColor: UIColor?
    @IBInspectable open var contentViewShadowOffset: CGSize = CGSize.zero
    @IBInspectable open var contentViewShadowOpacity: Float = 0.4
    @IBInspectable open var contentViewShadowRadius: CGFloat = 8.0
    @IBInspectable open var contentViewFadeOutAlpha: CGFloat = 1.0
    @IBInspectable open var contentViewScaleValue: CGFloat = 0.7
    @IBInspectable open var contentViewInLandscapeOffsetCenterX : CGFloat = 30.0
    @IBInspectable open var contentViewInPortraitOffsetCenterX: CGFloat = 30.0
    @IBInspectable open var parallaxMenuMinimumRelativeValue: CGFloat = -15
    @IBInspectable open var parallaxMenuMaximumRelativeValue: CGFloat = 15
    @IBInspectable open var parallaxContentMinimumRelativeValue: CGFloat = -25
    @IBInspectable open var parallaxContentMaximumRelativeValue: CGFloat = 25
    @IBInspectable open var parallaxEnabled: Bool = true
    @IBInspectable open var bouncesHorizontally: Bool = true
    @IBInspectable open var menuPrefersStatusBarHidden: Bool = false
    
    open var animationDuration: TimeInterval =  0.35
    open var menuViewControllerTransformation: CGAffineTransform?
    open var panGestureEnabled: Bool = false
    open var panFromEdge: Bool = false
    open var panMinimumOpenThreshold: Float = 60.0
    open var menuPreferredStatusBarStyle: UIStatusBarStyle = UIStatusBarStyle.default
    open var contentViewController: UINavigationController!
    ...
    convenience init(contentViewController: UIViewController, leftMenuViewController: UIViewController, rightMenuViewController: UIViewController) {
        self.init()
        self.contentViewController = contentViewController as! UINavigationController
    ...
    override open func awakeFromNib() {
        ...
        if (self.contentViewStoryboardID != nil) {
            self.contentViewController = self.storyboard!.instantiateViewController(withIdentifier: self.contentViewStoryboardID!) as! UINavigationController
        }
    ...
    open func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
        if (self.interactivePopGestureRecognizerEnabled && self.contentViewController!.isKind(of: UINavigationController.self)) {
        ...
        if (self.panFromEdge && gestureRecognizer.isKind(of: UIPanGestureRecognizer.self) && !self.visible) {
...
//                }
    ...
    override open var preferredStatusBarStyle : UIStatusBarStyle {
        ...
        if (self.contentViewContainer.frame.origin.y > 10) {
            ...
            statusBarStyle = self.menuPreferredStatusBarStyle //self.contentViewController!.preferredStatusBarStyle
        }
