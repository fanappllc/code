...
//  FanPhotographer
//
//  Created by Urvashi Bhagat on 1/31/18.
...
class ChatVC: UIViewController,UITextViewDelegate {
    ...
    func sendMessage(handler: @escaping() -> Void) {
        FanHUD.show()
        let param: HTTPParameters = ["to_id" : order.userId,
                                     "order_id"  : order.id,
                                     "message" : self.txtMessage.text!]
        
        APIComponents.Photographer.sendMessage(parameter: param) { [weak self] (success, data, error) in
            FanHUD.hide()
    ...
    func getMessageList(handler: @escaping() -> Void) {
        FanHUD.show()
        APIComponents.Photographer.getMessageList(orderID: order.id) { [weak self] (success, data, error) in
            FanHUD.hide()
            guard let strongSelf = self else { return }
            guard success, error == nil else {
                if let message = error?.localizedDescription {
